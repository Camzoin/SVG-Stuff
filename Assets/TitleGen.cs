        ...
        for (int jk= 0; jk < runXTimes; jk++)
        {
            ...
            foreach (Vector2 size in pieceSizeInInches)
            {
                ...
                for (int i = 0; i < piecesInSeries; i++)
                {
                    ...
                    svgVis.GenerateSVG(finalCompleteLineList, false, false, i.ToString(), Color.black, docSize, pieceName.Replace(" ", ""), "_TitleCard_" + size.x + "x" + size.y + "_", false);
                }
            }
        }
    }

    [ContextMenu("New Name")]
    public void NewName()
    {
        pieceName = svgVis.textImporter.GenerateRandomName();
    }
